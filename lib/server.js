var URL = require('url')
var http = require('http')
var cuid = require('cuid')
var Corsify = require('corsify')
var sendJson = require('send-data/json')
var ReqLogger = require('req-logger')
var healthPoint = require('healthpoint')
var HttpHashRouter = require('http-hash-router')
var redis = require('./redis')
if (process.env.NODE_ENV === 'development') redis.select(2)
if (process.env.NODE_ENV === 'test') redis.select(3)

var logic = require('./index')

var version = require('../package.json').version

var router = HttpHashRouter()
var logger = ReqLogger({ version: version })
var health = healthPoint({ version: version }, redis.healthCheck)
var cors = Corsify({
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Headers': 'authorization, accept, content-type'
})

router.set('/favicon.ico', empty)
router.set('/health', function health (req, res) {
  res.end('OK')
})
router.set('/route', async (req, res) => {
  const reqdata = []

  req.on('data', (data) => {
    reqdata.push(data)
  })
  req.on('end', async (data) => {
    var formdata = await JSON.parse(reqdata)

    const resdata = await logic.postRoute(formdata)

    res.write(JSON.stringify(resdata))
    res.end()
  })
})

router.set('/api/target/:id', async (req, res, requery) => {
  const reqdata = []

  req.on('data', (data) => {
    reqdata.push(data)
  })
  req.on('end', async (data) => {
    var formdata = await JSON.parse(reqdata)

    const resdata = await logic.updateTarget(requery.params.id, formdata)

    res.end(JSON.stringify({ status: resdata.msg }))
  })
})
router.set('/api/get_byid/:id', async (req, res, requery) => {
  const resdata = await logic.getTargetById(requery.params.id)

  res.writeHead(200, { 'Content-Type': 'application/json' })
  res.write(JSON.stringify(resdata))
  res.end()
})
router.set('/api/targets', async (req, res) => {
  const reqdata = []
  console.log('targets req.method', req.method)
  if (req.method === 'GET') {
    const resdata = await logic.getTargets('Targets')
    res.writeHead(200, { 'Content-Type': 'application/json' })
    res.write(JSON.stringify(resdata))
    res.end()
  } else {
    req.on('data', (data) => {
      reqdata.push(data)
    })
    req.on('end', async (data) => {
      var formdata = await JSON.parse(reqdata)
      console.log('targets formdata', formdata)
      const resdata = await logic.newTarget(formdata)

      res.write(JSON.stringify({ status: resdata.msg }))
      res.end()
    })
  }
})

module.exports = function createServer () {
  return http.createServer(cors(handler))
}

function handler (req, res) {
  if (req.url === '/health') return health(req, res)
  req.id = cuid()
  logger(req, res, { requestId: req.id }, function (info) {
    info.authEmail = (req.auth || {}).email
    console.log(info)
  })
  router(req, res, { query: getQuery(req.url) }, onError.bind(null, req, res))
}

function onError (req, res, err) {
  if (!err) return

  res.statusCode = err.statusCode || 500
  logError(req, res, err)

  sendJson(req, res, {
    error: err.message || http.STATUS_CODES[res.statusCode]
  })
}

function logError (req, res, err) {
  if (process.env.NODE_ENV === 'test') return

  var logType = res.statusCode >= 500 ? 'error' : 'warn'

  console[logType](
    {
      err: err,
      requestId: req.id,
      statusCode: res.statusCode
    },
    err.message
  )
}

function empty (req, res) {
  res.writeHead(204)
  res.end()
}

function getQuery (url) {
  return URL.parse(url, true).query; // eslint-disable-line
}
